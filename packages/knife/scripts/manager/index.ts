...
import type { Component, ComponentConstructorOptions, ComponentRenderItemInfo } from '@pokemonon/temanager'
import { Module } from '@pokemonon/temanager'
...
function createManager(map: MapItem[]) {
  ...
  function addComponent(opts: Partial<ComponentConstructorOptions>) {
    ...
    if (!component) {
      ...
      console.log(`${info.name} component exist`)
    }
  ...
  function removeComponent(name: string) {
    ...
    if (component) {
      ...
      console.log(`${name} component not exist`)
    }
  }
...
program.command('create')
  ...
  .action((functionName, options) => {
    const { group, module } = options
    if (!functionName || !group || !module) {
      throw new Error('miss params')
    }
    const parentCommon = manager.module.getComponent(group)
    const parentArray = manager.module.getComponent([group, module].join('.'))
...
program.command('remove')
  ...
  .action((functionName, options) => {
    ...
    manager.removeComponent([group, module, functionName].join('.'))
  })
